...
The UI never treats `ACCEPTED` or `FINALIZED` alone as execution success. A write is shown as successful only when its receipt is finalized and either the transaction result is `FINISHED_WITH_RETURN` or the authoritative leader receipt reports `SUCCESS`. Polling continues for up to 30 minutes; if that client-side window expires, the UI keeps the transaction in a submitted state, tells the operator not to resubmit, and links to Explorer for authoritative verification.
...
- `contracts/ReleaseLens.py` — exact Intelligent Contract source used by the DApp.
- `TESTING.md` — contract/runtime test plan.
- `assets/ReleaseLens_Logo.png` — submission and project logo.
